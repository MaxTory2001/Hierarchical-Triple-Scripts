...
void print_stable(float stable);
...
int n = 0; float inc; float a_over_rh; float mass_ratio = 0.0; double M;
...
int main(int argc, char* argv[]){
    ...
    if (argc > 1){
        ...
        if (argc == 4) {
            ...
            // random inc spaced probabilistically
            inc = strtof(argv[2], NULL);
	    inc *= 180/M_PI;
...
void loop_sma(){
     ...
     *
    */
    for (a_over_rh = 0.2; a_over_rh <= 1.2; a_over_rh += 0.01){
        float stable = 0;
        static int unstable = 0;
        if (a_over_rh <= 0.3) unstable = 0;
	
        // Once we have encountered more than 5 fully unstable orbits, we assume everything above is also unstable
        if (unstable >= 5){
            printf("all unstable :(");
            print_stable(0.0);
            continue;
        ...
        if (a_over_rh < predict_stable(inc, mass_ratio) / r_hill - 0.3){
            printf("all stable :)");
            print_stable(1.0);
            continue;
        }
        if (a_over_rh > predict_stable(inc, mass_ratio) / r_hill + 0.5){
            printf("all unstable :(");
            print_stable(0.0);
            unstable++;
            continue;
...
void generate_sims(struct reb_simulation* r){
    ...
    // initialise at apoapsis for more precision
    struct reb_particle m2 = reb_tools_orbit_to_particle(r->G, r->particles[0],  1.0e-2 * r->particles[0].m, a_over_rh * r_hill, ecc, inc * M_PI / 180, 0.0, 0.0, M_PI);
    m2.hash = reb_hash("m2");
    reb_add(r, m2);

    // The star
    reb_add_fmt(r, "m a theta", 1.0, a_out, f);
    reb_move_to_com(r);
...
void heartbeat(struct reb_simulation* r){
    ...
    // exit if the particle comes too close or escapes
    if (d_in > a_out / 2) {
		r->status = 1;
    }
}
...
void print_stable(float stable_frac){
    ...
    sprintf(fname, "eccentricities_in/stable_orbits_%.2e_e_%.2f.txt", mass_ratio, stop_ecc);
...
void print_time(struct reb_simulation* r){
    ...
    sprintf(fname, "eccentricities_in/time_0_M_%.2e_e_%.2f.txt", mass_ratio, stop_ecc);
