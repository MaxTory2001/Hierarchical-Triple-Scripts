...
for inc in incs:
    for a_over_rh in a_s:
        for i, axis in enumerate(axes):
            ...
            data_m2 = np.loadtxt(r"detailed_global_%.2f_%.2f_%d.txt" % (inc, a_over_rh, hashes[0]))
            data_m1 = np.loadtxt(r"detailed_global_%.2f_%.2f_%d.txt" % (inc, a_over_rh, hashes[1]))
            data_m3 = np.loadtxt(r"detailed_global_%.2f_%.2f_%d.txt" % (inc, a_over_rh, hashes[2]))
